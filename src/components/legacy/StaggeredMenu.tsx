            ...
            {items && items.length ? (
              items.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  {it.link.startsWith('/') ? (
                    <a href={it.link} className="sm-panel-item" aria-label={it.ariaLabel || it.label} data-index={idx + 1} onClick={closeMenu}>
                      <span className="sm-panel-itemLabel">{it.label}</span>
                    </a>
                  ) : (
                    <a className="sm-panel-item" href={it.link} aria-label={it.ariaLabel || it.label} data-index={idx + 1} onClick={closeMenu} target="_blank" rel="noopener noreferrer">
                      <span className="sm-panel-itemLabel">{it.label}</span>
                    </a>
                  )}
                </li>
